...
from .forms import LoginForm, UserRegistraction,ArticleRegistracionForm, ArticleUpdateForm
...
def user_login(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                return HttpResponse("You are Authentacited")
        else:
...
def register(request):
    if request.method == 'POST':
        user_form = UserRegistraction(request.POST)
        ...
        user_form = UserRegistraction()
    return render(request, 'account/register.html', {'user_form':user_form})

@login_required
def article_form(request):
    if request.method == 'POST':
        article_form = ArticleRegistracionForm(request.POST)        
        ...
        article_form = ArticleRegistracionForm()
